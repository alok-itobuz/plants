    ...
    <div class="collection-carousel-item h-100 w-100 position-absolute p-1 p-md-2 p-lg-3 " data-id=${i}>
        <div class="carousel-item-body h-100 w-100 d-flex flex-column">
            <div class="carousel-image-container flex-grow-1 overflow-hidden position-relative">
                <img class="object-fit-cover " src="../assets/static/collection/plant_${i % 3
    }.svg" alt="plant">
                <img class="object-fit-cover position-absolute start-0 top-0 w-100 h-100" src="../assets/static/collection/wrap_plant_${i % 3
    }.png" alt="plant">
            </div>
...
function createClientCard(i) {
  return `
    <div
        class="client-carousel-item ${i === 0 ? 'active' : ''} h-100 w-100 position-relative d-flex flex-column flex-md-row justify-content-around align-items-center p-4">
        <div class="client-image rounded-circle overflow-hidden">
            <img class="h-100 w-100" src="../assets/static/clients/client_1.svg" alt="" />
        </div>
        <div class="client-description ${i === 0 ? 'active' : ''} d-flex flex-column justify-content-center position-relative w-100">
...
const displayCarouselItem = (
  ...
  for (let i = 0; i < totalCarouselItem; i++) {
    ...
    currentItem.style.translate = `${(+currentItem.dataset.id - currentCarouselItem) * translateFactor
      }% ${translateY ? "-50%" : ""}`;
...
for (let i = 0; i < totalClientCarousalItem; i++) {
  if (i > 0) clientCarouselWrapper.insertAdjacentHTML('beforeend', createClientButtonWrapper())
...
clientCarouselContainer.addEventListener("click", (e) => {
  ...
  const descriptions = clientCarouselWrapper.querySelectorAll('.client-carousel-item .client-description')
